...
// ────────────────────────────────────────────────────────────────────────────
// Signup OTP — 6-digit code for new account email verification
// ────────────────────────────────────────────────────────────────────────────

/**
 * Send the signup confirmation as a 6-digit OTP code via Mailtrap.
 * Includes BOTH the code (so the user can type it) AND the magic link
 * (so the existing /auth/confirm click-flow keeps working).
 *
 * Generated by `supabaseAdmin.auth.admin.generateLink({ type: 'signup', email })`,
 * which returns `email_otp` and `action_link` in `data.properties`.
 */
export async function sendSignupOtpEmail(args: {
  to: string
  name?: string | null
  code: string
  ...
  return safeSend({
    to: args.to,
    subject: `Your VendoorX verification code: ${args.code}`,
    html: layout({
      preheader: `Your VendoorX verification code is ${args.code}. Expires in 10 minutes.`,
      iconEmoji: '🔐',
      iconBg: '#16a34a22',
      iconBorder: '#16a34a44',
      title: 'Verify your email',
      subtitle: 'VendoorX Account Setup',
      bodyHtml: `
        <p style="color:#111827;font-size:15px;line-height:1.6;margin:0 0 14px">${greeting}</p>
        <p style="color:#374151;font-size:14px;line-height:1.7;margin:0 0 18px">
          Welcome to VendoorX! Use the code below to verify your email address and finish setting up your account.
        </p>
        <div style="background:#f0fdf4;border:2px dashed #16a34a;border-radius:14px;padding:22px;margin-bottom:18px;text-align:center">
          <p style="color:#15803d;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;margin:0 0 10px">Verification code</p>
          <p class="vx-pw" style="font-family:'Courier New',Consolas,monospace;font-size:32px;font-weight:900;color:#0a0a0a;letter-spacing:6px;margin:0;line-height:1.1">${esc(args.code)}</p>
          <p style="color:#15803d;font-size:11px;margin:10px 0 0">Expires in 10 minutes</p>
        </div>
        ${args.confirmUrl ? `
          <p style="color:#6b7280;font-size:13px;line-height:1.6;margin:0 0 12px;text-align:center">
            Or click the button below to verify instantly:
          </p>
          <a href="${args.confirmUrl}" style="display:block;background:#16a34a;color:#fff;text-align:center;padding:14px;border-radius:12px;font-size:15px;font-weight:700;text-decoration:none;margin-bottom:18px">
            ✅ Verify my email
          </a>
        ` : ''}
        <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:14px 18px;margin-bottom:8px">
          <p style="color:#991b1b;font-size:13px;margin:0;line-height:1.6">
            <strong>Didn't sign up?</strong> You can safely ignore this email — no account will be created.
          </p>
        </div>
      `,
      footerNote: 'For your security, never share this code with anyone — not even VendoorX staff.',
    }),
  })
}
