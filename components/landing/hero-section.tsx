...
import {
  ...
  TrendingUp, Trophy, type LucideIcon,
} from 'lucide-react'
...
export function HeroSection({ user, settings, visitorCampus }: HeroSectionProps) {
  ...
  function renderSubtitle() {
    if (heroSubtitle.includes('{campuses}')) {
      ...
      return (<>{before}<span className="text-primary font-bold">{campusList.join(', ')}</span>{after}</>)
    }
    const braceMatch = heroSubtitle.match(/^([\s\S]*?)\{([^}]+)\}([\s\S]*)$/)
    if (braceMatch) {
      return (<>{braceMatch[1]}<span className="text-primary font-bold">{braceMatch[2]}</span>{braceMatch[3]}</>)
    }
    return heroSubtitle
  }
  ...
  return (
    ...
    >
      ...
      {/* ── Split layout ── */}
      <div className="relative w-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 pt-28 pb-16 flex flex-col lg:flex-row items-center gap-10 lg:gap-14">

        {/* ═══════════════ LEFT — text column ═══════════════ */}
        <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left gap-5 w-full">

          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-card border border-border shadow-sm text-sm text-muted-foreground font-medium">
          ...
          {/* Headline */}
          <h1 className="text-4xl sm:text-5xl lg:text-[3.5rem] xl:text-[4rem] font-black tracking-tight leading-[1.06] text-balance">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            {isAuthed ? (
              <>
                <Button size="lg" className="group rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold px-8 text-base shadow-lg shadow-primary/20 transition-all duration-300 hover:shadow-xl hover:shadow-primary/30 hover:scale-[1.02] w-full sm:w-auto" asChild>
                ...
                <Button size="lg" variant="outline" className="rounded-2xl font-semibold px-8 text-base hover:bg-muted/50 transition-all w-full sm:w-auto" asChild>
          ...
          {/* Social proof */}
          <div className="flex flex-col sm:flex-row items-center lg:items-start gap-3 pt-5 border-t border-border/50 w-full max-w-lg">
        ...
        <div className="hidden lg:flex flex-1 w-full max-w-md xl:max-w-lg flex-col gap-3">

          {/* Top row: earnings card + top seller card */}
          <div className="flex gap-3">
            {/* Earnings card */}
            <div className="flex-1 flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-card border border-border shadow-md">
              <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                <TrendingUp className="w-4 h-4 text-primary" />
              </div>
              <div className="min-w-0">
                <p className="text-[11px] text-muted-foreground font-medium leading-none mb-1">Today&apos;s earnings</p>
                <p className="text-lg font-black text-foreground leading-none">₦287,500</p>
              </div>
              <span className="ml-auto px-2 py-0.5 rounded-full bg-primary/10 text-[11px] font-bold text-primary shrink-0">+12%</span>
            </div>

            {/* Top seller card */}
            <div className="flex items-center gap-2.5 px-4 py-3.5 rounded-2xl bg-card border border-border shadow-md">
              <div className="w-9 h-9 rounded-xl bg-amber-50 dark:bg-amber-950/30 flex items-center justify-center shrink-0">
                <Trophy className="w-4 h-4 text-amber-600" />
              </div>
              <div className="min-w-0">
                <p className="text-[11px] text-muted-foreground font-medium leading-none mb-1">Top seller</p>
                <p className="text-base font-black text-foreground leading-none">₦1.2M / wk</p>
              </div>
            </div>
          </div>

          {/* Live activity mockup card */}
          <div className="w-full rounded-2xl border border-border bg-card shadow-xl shadow-black/5 overflow-hidden">
            {/* Header bar */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/40">
              ...
              <span className="text-xs font-semibold text-muted-foreground">Live on VendoorX</span>
          ...
          {/* Escrow badge row */}
          <div className="flex items-center gap-2.5 px-4 py-3 rounded-2xl bg-primary/5 border border-primary/15">
            <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center shrink-0">
              <CheckCircle2 className="w-4 h-4 text-primary-foreground" />
            </div>
            <div>
              <p className="text-xs font-bold text-foreground leading-tight">Escrow protected on every transaction</p>
              <p className="text-[11px] text-muted-foreground leading-tight">Your money is held safely until delivery is confirmed</p>
            </div>
          </div>
        </div>
      </div>
