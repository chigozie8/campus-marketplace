...
export function BlogPostEditor({
  ...
  const fileInputId = useRef(`cover-upload-${Math.random().toString(36).slice(2)}`)
  ...
  async function save(publishStatus?: 'draft' | 'published') {
    ...
    setSuccess('')
    if (!data.title.trim()) {
      setError('Post title is required before saving.')
      return
    }
    if (!data.content.trim()) {
      setError('Post content cannot be empty. Write something first!')
      return
    }
    ...
    try {
      ...
      const res     = await fetch(
        isEdit ? `/api/admin/blog/${data.id}` : '/api/admin/blog',
        {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="xl:hidden sticky top-0 z-30 bg-white/95 dark:bg-card/95 backdrop-blur border-b border-border px-4 py-3 flex items-center gap-2 -mx-4 sm:-mx-6">
        ...
        <button
          onClick={() => save('draft')}
          disabled={saving}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-border text-xs font-bold disabled:opacity-50 hover:bg-muted transition-colors"
        >
        ...
        <button
          ...
          disabled={saving}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-primary text-white text-xs font-bold disabled:opacity-50 hover:opacity-90 transition-opacity"
        >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="rounded-2xl border border-border bg-card p-6 min-h-[400px]">
              {data.content ? (
                <div className="prose prose-neutral dark:prose-invert max-w-none">
                  <p className="text-xs text-muted-foreground/60 bg-muted/30 p-3 rounded-lg mb-6 font-mono">
                    Preview — formatted on the public blog page
                  </p>
          ...
          {activeTab === 'seo' && (
            <div className="rounded-2xl border border-border bg-card p-5 space-y-5">
              <div>
                ...
                />
                <p className={`text-xs mt-1 ${data.seo_title.length > 60 ? 'text-amber-500' : 'text-muted-foreground'}`}>{data.seo_title.length}/60</p>
              </div>
              <div>
                ...
                />
                <p className={`text-xs mt-1 ${data.seo_description.length > 160 ? 'text-amber-500' : 'text-muted-foreground'}`}>{data.seo_description.length}/160</p>
              </div>
        ...
        <div className="space-y-4">
          ...
          {/* Publish — desktop only (mobile uses sticky bar) */}
          <div className="hidden xl:block rounded-2xl border border-border bg-card p-5 space-y-3">
          ...
          <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
            ...
            {/* Hidden file input — triggered via <label> (works on all mobile browsers) */}
            <input
            ...
            {/* Upload button — uses <label> for the file input, most reliable cross-browser */}
            <label
              htmlFor={fileInputId.current}
              className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed cursor-pointer transition-all text-sm font-semibold select-none ${
                uploadingImage
                  ? 'border-primary/40 bg-primary/5 text-primary opacity-60 cursor-not-allowed pointer-events-none'
            ...
            <div className="relative mt-1">
              <span className="absolute inset-x-0 -top-2.5 flex items-center justify-center pointer-events-none">
          ...
          <div className="xl:hidden rounded-2xl border border-border bg-card p-5">
