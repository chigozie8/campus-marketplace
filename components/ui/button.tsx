...
import { useGsapButton } from '@/hooks/use-gsap-button'
...
type ButtonProps = React.ComponentProps<'button'> &
  VariantProps<typeof buttonVariants> & {
    asChild?: boolean
    /** Set to false to opt out of GSAP hover/press animations. */
    animate?: boolean
  }
...
function Button({
  ...
  // Skip GSAP for the 'link' variant (text-only) and disabled buttons.
  const shouldAnimate =
    animate && !disabled && !asChild && variant !== 'link'
  const animRef = useGsapButton<HTMLButtonElement>(!shouldAnimate)

  return (
    <Comp
      ref={shouldAnimate ? (animRef as React.Ref<HTMLButtonElement>) : undefined}
      data-slot="button"
      disabled={disabled}
