...
export async function POST(
  ...
  const { data: verification, error: fetchErr } = await adminClient
    ...
    .eq('id', id)
    .single()
  if (fetchErr || !verification) return NextResponse.json({ error: 'Verification not found' }, { status: 404 })
  const { id_image_url, selfie_image_url } = verification
  if (!id_image_url || !selfie_image_url) {
    return NextResponse.json({ error: 'Both an ID photo and a selfie are required for face matching.' }, { status: 400 })
  }
