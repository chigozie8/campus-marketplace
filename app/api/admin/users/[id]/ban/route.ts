...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const isBanned = targetUser.user.banned_until && new Date(targetUser.user.banned_until) > new Date()
  if (action === 'ban' && !isBanned) {
    const { error } = await adminClient.auth.admin.updateUserById(targetUserId, { ban_duration: '876600h' })
