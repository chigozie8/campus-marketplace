...
export default function WalletError({
  ...
  useEffect(() => {
    console.error('[VendoorX] Wallet error:', error.message)
  }, [error])
  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center px-6 text-center">
      <div className="w-16 h-16 rounded-2xl bg-card border border-border shadow-sm flex items-center justify-center mb-5">
        <Wallet className="w-7 h-7 text-[#16a34a]" strokeWidth={1.5} />
      </div>
      ...
      <p className="text-muted-foreground text-sm max-w-xs leading-relaxed mb-7">
        We couldn&apos;t load your wallet right now. Your balance is safe — this is a display issue only. Please try again.
      </p>
