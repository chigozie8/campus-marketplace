/**
 * Builds formatted WhatsApp reply strings for each intent outcome.
 */

export function buildWelcome() {
  return (
    `Welcome to *VendorX Marketplace*!\n\n` +
    `I can help you:\n` +
    `- Search for products (e.g., "shoes", "phone")\n` +
    `- Get product prices (e.g., "price of Airpods")\n` +
    `- Buy a product (e.g., "BUY <product-id>")\n\n` +
    `What are you looking for today?`
  )
}

export function buildProductList(products) {
  if (!products || products.length === 0) {
    return `Sorry, I couldn't find any products matching your search. Try a different keyword.`
  }

  const lines = products.map((p) => {
    const vendor = p.profiles?.full_name ?? 'Unknown Vendor'
    const price = Number(p.price).toLocaleString('en-NG', { style: 'currency', currency: 'NGN' })
    return (
      `*${p.name}*\n` +
      `Price: ${price}\n` +
      `Vendor: ${vendor}\n` +
      `${p.description ? p.description.slice(0, 80) + '...' : ''}\n` +
      `Reply *BUY ${p.id}* to purchase\n`
    )
  })

  return `Here are some products for you:\n\n` + lines.join('\n---\n\n')
}

export function buildSingleProduct(product) {
  const vendor = product.profiles?.full_name ?? 'Unknown Vendor'
  const price = Number(product.price).toLocaleString('en-NG', { style: 'currency', currency: 'NGN' })

  return (
    `*${product.name}*\n` +
    `Price: ${price}\n` +
    `Vendor: ${vendor}\n` +
    `${product.description ?? ''}\n\n` +
    `Reply *BUY ${product.id}* to purchase`
  )
}

export function buildBuyPrompt() {
  return `To buy a product, reply with: *BUY <product-id>*\n\nSearch for products first by typing a keyword like "shoes" or "phone".`
}

export function buildOrderCreated(order) {
  const price = Number(order.total_amount).toLocaleString('en-NG', { style: 'currency', currency: 'NGN' })
  return (
    `Your order has been created!\n\n` +
    `*Order ID:* ${order.id}\n` +
    `*Product:* ${order.product?.name ?? order.product_id}\n` +
    `*Total:* ${price}\n` +
    `*Status:* ${order.status}\n\n` +
    `Please log in to the VendorX app to complete payment.`
  )
}

export function buildOrderStatusHelp() {
  return `To check your order status, please log in to the VendorX app and visit *My Orders*.`
}

export function buildHelp() {
  return (
    `I didn't quite understand that. Here's what you can do:\n\n` +
    `- Type a product name to search (e.g., "iPhone")\n` +
    `- Type *BUY <id>* to start a purchase\n` +
    `- Type *hi* to restart\n\n` +
    `Need more help? Contact support.`
  )
}

export function buildError() {
  return `Oops! Something went wrong on our end. Please try again in a moment.`
}
